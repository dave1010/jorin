...
func ToolsManifest() (list []types.Tool) {
	return []types.Tool{
		...
		{Type: "function", Function: types.ToolFunction{
			Name:        "apply_patch",
			Description: "Apply a patch to a file to create, update, delete, or move it. Supports unified diff (---/+++) or '*** Begin Patch' format (with *** Add File / *** Update File / *** Delete File / *** Move to). Unified diff MUST start with '--- filename' and '+++ filename' (or '/dev/null'). Context lines must start with a space. Example update:\n--- a/README.md\n+++ b/README.md\n@@ -1,1 +1,1 @@\n-Old text\n+New text\n unchanged context",
